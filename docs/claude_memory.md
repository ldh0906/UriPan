...
- **달력 평면 그리드 전환 (2026-06-03, Codex·Claude검증, 커밋 32d7860)**: 사용자가 "연속 일정이 점점점 말고 이어지게" 요청 → 날짜 카드 섬(테두리/배경/간격) 제거하고 **구글캘린더식 평면 격자**로. 멀티데이 일정 = 칸 가로지르는 **연속 막대**(주간·월간 둘 다, 주 경계는 각진 끝으로 이어짐). 순수함수 `calendarWeekBars`(lane 패킹, 최대 3 lane) + 테스트. 할일=앰버 점 유지, schedule 카운트 점은 막대로 대체. 테스트 120→127. (스펙 `docs/codex-tasks/02-calendar-flat-grid-bars.md`.) 실기기 비주얼 미확인.
