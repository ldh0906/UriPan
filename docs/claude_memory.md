...
> 마지막 갱신: 2026-06-02 (코멘트/댓글 페이즈 출시)
...
- **코멘트/댓글 페이즈 (5태스크: 01 스키마 / 01.5 테스트 시각주입 / 02 모델+repo+친절시간 / 03 상세시트 UI+카드 배지 / 04 스레드 실시간)**: 첫 스키마 변경(item_comments 테이블+RLS+realtime publication). BoardComment 모델, repository CRUD(Memory+Supabase), friendlyRelativeTime(방금/N분 전/N시간 전/어제/M.D), CommentThread 위젯(목록·입력·삭제확인·빈상태·친절에러), 카드 💬 배지(비실시간 카운트), 상세시트 열림 동안 item별 실시간 구독. UI 라벨은 "댓글"(네이티브 한국어). 테스트 85→100, analyze 무이슈. 큐 소진 후 삭제. 주의: (a) item_comments 마이그레이션은 원격 Supabase에 아직 미적용(supabase db push 필요), (b) 댓글 에러 시 board 배너+스레드 인라인 이중 표시 — UI/UX 폴리시 때 정리 후보, (c) 실시간 E2E는 실제 2클라이언트 필요.

## 5. 다음 방향 (gstack 패널 결론, 2026-06-02)
판정: **"잘 만든 공유 보드"는 맞지만 아직 "가족 데일리 드라이버"는 아님.** 결정적 공백 순서:
1. ~~**알림/리마인더**~~ (Phase A 완료, Phase B=FCM 남음)
2. ~~**코멘트/가벼운 소통**~~ ← **출시 완료 (2026-06-02)**. 설계: `docs/superpowers/specs/2026-06-02-item-comments-design.md`, (실행 큐는 소진 후 삭제). 결정: 스레드 실시간 / 코멘트 수 배지 비실시간(C안) / 첫 스키마 변경(item_comments) / FCM은 Phase B. 코멘트 이후 = UI/UX 폴리시.
   다음 = **UI/UX 폴리시** (대회의 결정). 후보 메모: 댓글 에러 이중표시 정리, 전반 비주얼 다듬기.
3. **계정 복구(비번 재설정) + 배포(가족 폰 설치)** — 현 상태 A(설치 0대). 네가 가족에게 실제 배포할 마음이 생기는 순간 1순위로 승격(합성 이메일 구조라 비번 재설정 선행 필요).
4. 반복 일정/할 일
